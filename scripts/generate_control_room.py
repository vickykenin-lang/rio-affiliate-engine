...
def main():
    ...
    for offer, meta in reversed(list(published.items())):
        ...
        visual=(f'<img src="{esc(image)}" loading="lazy" alt="{esc(meta.get("product_name"))}">' if image else '<div class="noimg">Preview pending telemetry</div>')
...
body{{font-family:system-ui;margin:0;background:#0d1117;color:#e6edf3}}main{{max-width:1280px;margin:auto;padding:24px}}a{{color:#58a6ff}}.top{{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin:18px 0}}.card,.panel,.post{{background:#161b22;border:1px solid #30363d;border-radius:14px}}.card{{padding:16px}}.card small,.post small{{display:block;color:#8b949e}}.card strong{{font-size:28px}}.panel{{padding:18px;margin:16px 0}}.posts{{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px}}.post{{overflow:hidden}}.post img,.noimg{{width:100%;height:280px;object-fit:cover;background:#21262d;display:flex;align-items:center;justify-content:center;color:#8b949e}}.pad{{padding:12px}}.metrics{{font-size:13px;color:#8b949e;margin:8px 0}}.actions a{{display:inline-block;margin:4px 8px 4px 0;padding:10px 12px;border:1px solid #30363d;border-radius:9px;text-decoration:none}}code{{white-space:pre-wrap}}</style></head><body><main>
<div class="top"><div><h1>RIO Control Room</h1><p>Revenue, posts, website, telemetry and live development in one place.</p></div><div class="actions"><a href="../">Open Live Website</a><a href="../dashboard/">CEO Dashboard</a><a href="../products/">Products</a><a href="../guides/">Guides</a><a href="../blog/">Blog</a></div></div>
...
<section class="panel"><h2>Published Instagram Posts</h2><div class="posts">{post_html}</div></section>
<section class="panel"><h2>Website & Content</h2><div class="actions"><a href="../" target="_blank">Homepage</a><a href="../products/" target="_blank">Product pages</a><a href="../guides/" target="_blank">Buying guides</a><a href="../articles/" target="_blank">Articles</a><a href="../compare.html" target="_blank">Compare page</a></div></section>
    ...
    print('RIO Control Room generated')
